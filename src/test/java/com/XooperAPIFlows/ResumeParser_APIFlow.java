...
public class ResumeParser_APIFlow extends BaseClass {
	...
	public void resumeParseUploadFile() {
		...
		try {
			...
			ExtentReportManager.startTest("Resume Parser API Testing Started");
			...
			Assert.assertNotNull(parserId, "Resume Parser ID should not be null in the response.");
			System.out.println("Expected Resume Parser_id = " + parserId);
			...
			ExtentReportManager.log(Status.INFO,
					"Resume Parser test data successfully stored in extracted_resumes table: " + doc.toJson());
			...
			ExtentReportManager.log(Status.PASS, "MongoDB Resume Parser_id matches as expected: " + parserId);
